...
def cobs_encode(data):
    ...
    code_index = 0
    code = 1
    output = (len(data) + 1 + (len(data) / 254)) * [chr(0)]

    while read_index < len(data):
        if data[read_index] == chr(0):
            output[code_index] = chr(code)
            code = 1
    ...
    return ''.join(output[:write_index])
...
def cobs_decode(data):
    ...
    write_index = 0
    code = 0
    output = len(data) * [chr(0)]

    while read_index < len(data):
        code = ord(data[read_index])
        ...
        if not code == 0xff and not read_index == len(data):
            output[write_index] = chr(0)
            write_index += 1

    return ''.join(output[:write_index])

if __name__ == '__main__':
    dat = (chr(0x11), chr(0x00), chr(0x22), chr(0x42), chr(0x00), chr(0xff), chr(0x00))
    print "Original Length: {0} | {1}".format(len(dat), [hex(ord(x)) for x in dat])

    res = cobs_encode(dat)
    print "Encoded length:  {0} | {1}".format(len(res), [hex(ord(x)) for x in res])

    dat = list(res)

    res = cobs_decode(dat)
    print "Decoded Length:  {0} | {1}".format(len(res), [hex(ord(x)) for x in res])
